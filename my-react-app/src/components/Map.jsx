      ...
      {gameState.provinces?.map((p, i) => {
        ...
        return (
          <g key={`label-${p.province_id}`} transform={`translate(${x},${y})`}>
            {icons.length > 0 && <text y={-6} textAnchor="middle" fontSize="12">{icons.join(" ")}</text>}
            <text y={10} textAnchor="middle" fontSize="10" fill="#222">{p.name}</text>
          </g>
        );
      })}
    </svg>
